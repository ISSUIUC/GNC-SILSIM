...
public:
    Atmosphere(
        ...
        Vec3   nominal_wind_direction             = {0.0, 0.0, 0.0},
    ...
    double get_geometric_to_geopotential(double altitude) const {
        double r = 6371000.0;
        return (r * altitude) / (r + altitude);
    }

    double get_temperature(double altitude) const {
        ...
        if (altitude_h < 11.0) {
            ...
            std::printf("Exceeding calculatable altitude!\n");
            temperature = -1.0;
        }
        return temperature;
    }

    double get_pressure(double altitude) const {
        double P_0 = 101325.0;
        double T_0 = 288.16;
        double b   = 0.0065;
        double g   = 9.81;
    ...
    double get_density(double altitude, bool noise = false, const Vec3& position = {0.0, 0.0, 0.0}) const {
        double R           = 287.053;
    ...
    double get_altitude(double pressure) const {
        double P_0 = 101325.0;
        double T_0 = 288.16;
        double b   = 0.0065;
        double g   = 9.81;
        double R   = 287.05;
        double pressure_ratio = pressure / P_0;
        return (T_0 / b) * (1.0 - std::pow(pressure_ratio, b * R / g));
    }

    double get_speed_of_sound(double altitude) const {
        double gamma        = 1.4;
        double gas_constant = 287.05;
        return std::sqrt(gamma * gas_constant * get_temperature(altitude));
    }

    Vec3 get_nominal_wind_direction() const { return nominal_wind_direction_; }
    double get_nominal_wind_magnitude() const { return nominal_wind_magnitude_; }

    void set_nominal_wind_direction(Vec3 direction) {
    ...
    Vec3 get_wind_vector(double t_stamp) const {
        double dir_alpha = 0.9997;
        double mag_alpha = 0.99;
        Vec3 generated_direction_variance = {0.0, 0.0, 0.0};
        double generated_magnitude_variance = 0.0;
        Vec3 current_wind_direction = nominal_wind_direction_;

        if (enable_direction_variance_) {
            if ((t_stamp - last_direction_variance_update_) >= direction_variance_update_rate_) {
                ...
                last_direction_variance_update_ = t_stamp;
            }
        ...
        if (enable_magnitude_variance_) {
            if ((t_stamp - last_magnitude_variance_update_) >= magnitude_variance_update_rate_) {
                ...
                last_magnitude_variance_update_ = t_stamp;
            }
...
private:
    void normalize_vector_inplace(Vec3& arr) const {
        double sum_sq = 0.0;
        for (double val : arr) {
            sum_sq += val * val;
        }
    ...
    Vec3   nominal_wind_direction_;
    double nominal_wind_magnitude_;

    mutable double last_direction_variance_update_;
    mutable double last_magnitude_variance_update_;
    double direction_variance_update_rate_;
    double magnitude_variance_update_rate_;

    mutable Vec3 direction_variance_vect_;
    mutable double magnitude_variance_val_;
};
